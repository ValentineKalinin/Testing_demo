...
public class WebMultipleBrowserTest implements IAbstractTest {
    ...
    public void multipleBrowserTest() {
        HomePageSample chromeHomePage = new HomePageSample(getDriver("chrome", new ChromeCapabilities().getCapability("Chrome Test")));
        chromeHomePage.open();
        Assert.assertTrue(chromeHomePage.isPageOpened(), "Chrome home page is not opened!");

        HomePageSample firefoxHomePage = new HomePageSample(getDriver("firefox", new FirefoxCapabilities().getCapability("Firefox Test")));
        ...
        NewsPageSample newsPage = chromeHomePage.getFooterMenu().openNewsPage();
